...
if (args.includes('--anim')) {
  // 动画帧：alpha 脉动（呼吸）——模板图明暗变化，间隔由 startTrayAnimation 控制
  const pulses = [1.0, 0.78, 1.0, 0.78]
  const outDir = join(RES, 'tray-anim')
  mkdirSync(outDir, { recursive: true })
  pulses.forEach((mul, i) => {
    const p = join(outDir, `frame-${i}.png`)
    writeFileSync(p, encodePng(SIZE, SIZE, drawBolt(SIZE, mul)))
    console.log(`[generate-tray] frame-${i}.png（alpha=${mul}）`)
  })
  console.log(`[generate-tray] 托盘呼吸动画已写入 ${outDir}`)
} else {
